...
When using 'message = FALSE' knitting the documment provides an easier document to read for reporting, as when
'message = TRUE' the few conflicts found when importing the tidyverse are also shown in the knitted report.
...
Similarly as before, turing the mmessage option off with 'message = FALSE' the knitted documment removes the console output of the code chuck for a more readable document.
...
To answer the question of lowest unemployment rate we would need the variable `unemployment_rate`.
Looking at the data from `unemployment_rate`, first we would want to arrange the data in ascending order by the unemployment rate of college majors. This would give the top major result of with the lowest unemployment rate from the dataset `college_recent_grads`. 
...
The major with the lowest unemployment rate is Mathematics and Computer Science. This major is tied at 0 with Military Technologies, Botany, Soil Science, and Educational Administration and Supervision, respectively.
...
The major with the highest proportion of women from the dataset college_recent_grads is shown to be Early Childhood Education, which was found by arranging the dataset in descending order by the variable sharewomen.
...
Based on the results given (0) there is not a single STEM category that falls below the median incom of college majors.
